...
//  https://github.com/alibaba/LazyScrollView/blob/master/LazyScrollView/TMMuiLazyScrollView.m
...
// https://stackoverflow.com/questions/16020918/avoid-extra-static-variables-for-associated-objects-keys/16020927#16020927
...
@interface GZLazyScrollViewObserver : NSObject

@property (nonatomic, weak) GZLazyScrollView *lazyScrollView;
...
@interface GZLazyScrollView() <UIScrollViewDelegate>
...
@property (nonatomic,weak) id <GZLazyScrollViewDelegate> lazyScrollViewDelegate;
...
@implementation GZLazyScrollView
...
-(void)setDelegate:(id<GZLazyScrollViewDelegate>)delegate
{
    if(!delegate)
    {
...
- (void)creatScrollViewIndex
{
    ...
    if(self.dataSource && [self.dataSource conformsToProtocol:@protocol(GZLazyScrollViewDataSource)] &&
    ...
    for (NSUInteger i = 0 ; i< count ; i++) {
        GZRectModel *rectmodel;
        if(self.dataSource
           &&[self.dataSource conformsToProtocol:@protocol(GZLazyScrollViewDataSource)]
    ...
    {
        CGRect rect1 = [(GZRectModel *) obj1 absoluteRect];
        CGRect rect2 = [(GZRectModel *) obj2 absoluteRect];
...
-(NSUInteger) binarySearchForIndex:(NSArray *)frameArray baseLine:(CGFloat)baseLine isFromTop:(BOOL)fromTop
{
    ...
    while (mid > min && mid < max) {
        CGRect rect = [(GZRectModel *)[frameArray objectAtIndex:mid] absoluteRect];
        // For top
        if(fromTop) {
            CGFloat itemTop = CGRectGetMinY(rect);
            if (itemTop <= baseLine) {
                CGRect nextItemRect = [(GZRectModel *)[frameArray objectAtIndex:mid + 1] absoluteRect];
...
-(NSSet *)showingItemIndexSetFrom :(CGFloat)startY to:(CGFloat)endY
{
    ...
    if (self.modelsSortedByBottom && self.modelsSortedByBottom.count > 0) {
        for (NSUInteger i = 0; i <= endBottomIndex; i++) {
            GZRectModel *model = [self.modelsSortedByBottom objectAtIndex:i];
            if (model != nil) {
    ...
    if (self.modelsSortedByTop && self.modelsSortedByTop.count > 0) {
        for (NSInteger i = 0; i <= endTopIndex; i++) {
            GZRectModel *model = [self.modelsSortedByTop objectAtIndex:i];
            if (model != nil) {
    ...
    for (NSString *muiID in itemShouldShowSet)
    {
        ...
        if(![self isCellVisible:muiID] || [self.shouldReloadItems containsObject:muiID])
        {
                ...
                [self.dataSource respondsToSelector:@selector(scrollView: itemByMuiID:)])
            {
                ...
                // Call afterGetView
                if ([viewToShow conformsToProtocol:@protocol(GZLazyScrollViewCellProtocol)] &&
                    [viewToShow respondsToSelector:@selector(mui_afterGetView)]) {
                    [(UIView<GZLazyScrollViewCellProtocol> *)viewToShow mui_afterGetView];
                }
                if (viewToShow)
                {
...
- (void)findViewsInVisibleRect
{
    ...
    for (UIView *view in self.visibleItems) {
        if (view && [itemViewSet containsObject:view.muiID]) {
            if ([view conformsToProtocol:@protocol(GZLazyScrollViewCellProtocol)] && [view respondsToSelector:@selector(mui_didEnterWithTimes:)]) {
                NSUInteger times = 0;
                ...
                [(UIView<GZLazyScrollViewCellProtocol> *)view mui_didEnterWithTimes:times];
            }
        }
    }
...
- (nullable UIView *)dequeueReusableItemWithIdentifier:(NSString *)identifier
{
    ...
    if ([view conformsToProtocol:@protocol(GZLazyScrollViewCellProtocol)] && [view respondsToSelector:@selector(mui_prepareForReuse)]) {
        [(UIView<GZLazyScrollViewCellProtocol> *)view mui_prepareForReuse];
    }
    return view;
}
