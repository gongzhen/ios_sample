//
//  LinkedListUnitTest.swift
//  LinkedListUnitTest
//
//  Created by zhen gong on 6/5/17.
//  Copyright © 2017 zhen gong. All rights reserved.
//

import XCTest

class LinkedListUnitTest: XCTestCase {
    ...
    func testInsertAtFront() {
        let list = buildList()
        let prev = list.node(atIndex: -1)
        let next = list.node(atIndex: 0)
        let nodeCount = list.count
        list.insert(444, atIndex: 0)
        let node = list.node(atIndex: 0)
        XCTAssertNotNil(node)
        XCTAssertEqual(node!.value, 444)
        XCTAssertEqual(nodeCount + 1, list.count)
    ...
    func testInsertAtLast() {
        let list = buildList()
        let nodeCount = list.count
        let last = list.node(atIndex: nodeCount-1)
        let next = list.node(atIndex: nodeCount)

        list.insert(444, atIndex: nodeCount-1)
        let node = list.node(atIndex: nodeCount-1)
        XCTAssertNotNil(node)
        XCTAssertEqual(node!.value, 444)
        XCTAssertEqual(nodeCount + 1, list.count)
        XCTAssertNil(next)
        XCTAssertFalse(last === node)
        XCTAssertFalse(next === node)
        XCTAssertTrue(last!.previous === node)
    }
